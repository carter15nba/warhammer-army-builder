...
public class createSQLUI extends javax.swing.JFrame {
    ...
    public createSQLUI() {
            ...
            tableEquipment.getColumnModel().getColumn(6).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(model, 6));
            model = new String[5];
            model[0] = "N/A";
            i=1;
            ...
            table.getColumnModel().getColumn(13).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(model, 13));
            utilTable.getColumnModel().getColumn(14).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(model, 14));
            model = new String[7];
            model[0] = "N/A";
            pos = 1;
            ...
            tableEquipment.getColumnModel().getColumn(5).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(model, 5));
        ...
        ccaseLayout.setVerticalGroup(
            ...
            .addGroup(ccaseLayout.createSequentialGroup()
                ...
                .addGroup(ccaseLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(jLabel2, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(jScrollPane9, javax.swing.GroupLayout.DEFAULT_SIZE, 284, Short.MAX_VALUE)
    ...
    private void generateSQLActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_generateSQLActionPerformed
        ...
        if(pane==0){
            ...
            myrmidia.File.SQLFileWriter.writeRaceUnitSQLFile(race, sql);
        }
        else if(pane==1){
            parseSQLFromUtilityTable();
            myrmidia.File.SQLFileWriter.writeUtilityUnitSQLFile(race,sql);
        }
        else if(pane==2){
            parseSQLFromUnit_UtilityTable();
            myrmidia.File.SQLFileWriter.write_Unit_UtilitySQLFile(race, sql);
        }
        else if(pane==3){
            parseSQLFromEquipmentTable();
            myrmidia.File.SQLFileWriter.writeEquipmentSQLFile(sql);
        }
        else if(pane==4){
            parseSQLFromUnit_EquipmentTable();
            myrmidia.File.SQLFileWriter.writeUnit_EquipmentSQLFile(race, sql);
        }
        else if(pane==5){
            parseSQLFromSpecialRules();
            myrmidia.File.SQLFileWriter.writeRule(sql);
        }
        else if(pane==6){
            parseSQLFromAssocUnit_Rule();
            myrmidia.File.SQLFileWriter.write_unit_ruleSQLFile(race, sql);
        }
        else if(pane==7){
            parseEQ_RuleTable();
            myrmidia.File.SQLFileWriter.write_eq_ruleSQLFile(sql);
        }
        else if(pane==8){
            parseArmyTable();
            myrmidia.File.SQLFileWriter.write_armySQLFile(race,sql);
        }
        else if(pane==9){
            parseCaseTable();
            myrmidia.File.SQLFileWriter.write_caseSQLFile(sql);
        }
        
    }//GEN-LAST:event_generateSQLActionPerformed

    private void loadButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_loadButtonActionPerformed
    JFileChooser jfc = new JFileChooser(new java.io.File("src/myrmidia/Database/Resources/"));
    jfc.setFileFilter(new FileFilter() {
        ...
        if(ret==JFileChooser.APPROVE_OPTION){
            emptyTable();
            ArrayList<String> result = myrmidia.File.SQLFileParser.parseSQL(jfc.getSelectedFile());
            if(pane==0)
    ...
    private void executeSQLActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_executeSQLActionPerformed
        try {
            ...
            for (String string : sql) {
                dbm.executeSQL(string, myrmidia.Database.DatabaseManager.UPDATE_QUERY);
            }
            dbm.commit();
        }
    ...
    private void formWindowClosing(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowClosing
        myrmidia.Database.DatabaseManager.getInstance().disconnectNoHibernate(false);
    ...
    private void tabbedPaneStateChanged(javax.swing.event.ChangeEvent evt) {//GEN-FIRST:event_tabbedPaneStateChanged
        ...
        try{
            if(pane==1){
                ...
                tableAssocUnit_Util.getColumnModel().getColumn(1).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(comp, 1));
                ...
                comp = util.toArray(comp);
                tableAssocUnit_Util.getColumnModel().getColumn(0).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(comp, 0));
            }
            else if(pane==3){
                ...
                String[] comp = new String[util.size()];
                comp = util.toArray(comp);
                tableAssocUnit_Equipment.getColumnModel().getColumn(1).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(comp, 1));
                util.clear();
                util.add("N/A");
                res = dbm.executeSQL("Select name from UNIT where race ='"+raceBox.getSelectedItem().toString()+"'", DatabaseManager.SELECT_QUERY);
                while(res.next())
                ...
                comp = new String[util.size()];
                comp = util.toArray(comp);
                tableAssocUnit_Equipment.getColumnModel().getColumn(0).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(comp, 0));
            }
            else if(pane==5){
                ...
                String[] comp = new String[util.size()];
                comp = util.toArray(comp);
                tableAssoc_unit_rule.getColumnModel().getColumn(0).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(comp, 0));
                util.clear();
                util.add("N/A");
                ...
                comp = new String[util.size()];
                comp = util.toArray(comp);
                tableAssoc_unit_rule.getColumnModel().getColumn(1).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(comp, 1));
            }
            else if(pane==7){
                ...
                String[] comp = new String[util.size()];
                comp = util.toArray(comp);
                tableAssocEqRule.getColumnModel().getColumn(0).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(comp, 0));
                util.clear();
                util.add("N/A");
                res = dbm.executeSQL("SELECT SPECIALRULE FROM SPECIALRULES ORDER BY SPECIALRULE ASC", DatabaseManager.SELECT_QUERY);
                while(res.next())
                ...
                comp = new String[util.size()];
                comp = util.toArray(comp);
                tableAssocEqRule.getColumnModel().getColumn(1).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(comp, 1));
            }
            else if(pane==8){
                ResultSet res = dbm.executeSQL("Select name from unit where race='"+raceBox.getSelectedItem().toString()+"'", DatabaseManager.SELECT_QUERY);
                ...
                tableArmy.getColumnModel().getColumn(0).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(comp, 0));
                ...
                tableCreateCase.getColumnModel().getColumn(1).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(cont, 1));
                ...
                tableCreateCase.getColumnModel().getColumn(2).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(race, 2));
                Outcomes[] out = Outcomes.values();
                String[] outcome = new String[out.length];
                for(int i = 0 ; i < out.length;i++){
                    outcome[i] = out[i].toString();
                }
                tableCreateCase.getColumnModel().getColumn(3).setCellRenderer(new myrmidia.UI.Resources.ComboBoxTableCellRenderer(outcome, 3));
